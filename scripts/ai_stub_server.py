...
async def completions(request: Request) -> dict:
    ...
    # planner call -> JSON plan; chat call -> echo a snippet of the context
    if "retrieval plan" in messages[0]["content"] if messages else False:
        answer = (
            '{"intent":"overdue_tasks","source_types":["task"],"time_preset":"all",'
            '"keywords":[],"mine_only":true}'
        )
    else:
        answer = (
            "根据你当前有权限访问的 LabFlow 数据："
            + ("、".join(part for part in last.split("EXPERIMENT_NO:") if part)[:0])
