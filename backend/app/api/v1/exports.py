"""CSV / XLSX exports, permission-controlled (PRD §27)."""
...
def _csv_response(rows: list[list], filename: str) -> StreamingResponse:
    ...
    data = io.BytesIO(buf.getvalue().encode("utf-8-sig"))  # BOM so Excel opens UTF-8 correctly
...
HEADERS = {
    "members": ["ID", "姓名", "用户名", "邮箱", "身份", "年级", "学号", "研究方向", "状态", "入组日期"],
    "weekly-reports": ["ID", "成员", "周起始", "周结束", "状态", "本周工作", "问题", "下周计划", "自评进度", "导师意见"],
    "tasks": ["ID", "任务", "项目", "负责人", "状态", "优先级", "截止日期", "进度", "创建时间"],
...
def export(
    ...
    if kind == "members":
        for m in db.scalars(
            select(MemberProfile).options(joinedload(MemberProfile.user)).order_by(MemberProfile.id)
        ).all():
            u = m.user
            rows.append(
                [m.id, u.name if u else "", u.username if u else "", u.email if u else "",
                 m.member_type, m.grade_year or "", m.student_no or "", m.research_direction or "",
                 m.status, m.join_date.isoformat() if m.join_date else ""]
            )
        ...
        for r in db.scalars(
            ...
            rows.append(
                [r.id, name, r.week_start.isoformat(), r.week_end.isoformat(), r.status,
        ...
        for t in db.scalars(
            select(Task).options(joinedload(Task.project)).where(Task.deleted_at.is_(None))
        ).all():
            assignee = db.get(User, t.assignee_id) if t.assignee_id else None
            rows.append(
                [t.id, t.title, t.project.name if t.project else "", assignee.name if assignee else "",
                 t.status, t.priority, t.due_date.isoformat() if t.due_date else "", t.progress,
                 t.created_at.strftime("%Y-%m-%d %H:%M")]
            )
    elif kind == "equipment":
        for e in db.scalars(select(Equipment).where(Equipment.deleted_at.is_(None))).all():
            manager = db.get(User, e.manager_id) if e.manager_id else None
            rows.append(
                [e.asset_no, e.name, e.category, e.manufacturer or "", e.model or "",
                 e.location or "", e.status, manager.name if manager else ""]
            )
    elif kind == "equipment-bookings":
        for b in db.scalars(
            select(EquipmentBooking).options(joinedload(EquipmentBooking.user)).order_by(EquipmentBooking.start_time.desc())
        ).all():
            eq = db.get(Equipment, b.equipment_id)
            rows.append(
                [b.id, eq.name if eq else "", b.user.name if b.user else "",
                 b.start_time.strftime("%Y-%m-%d %H:%M"), b.end_time.strftime("%Y-%m-%d %H:%M"),
    ...
    return _xlsx_response(rows, filename) if format == "xlsx" else _csv_response(rows, filename)
